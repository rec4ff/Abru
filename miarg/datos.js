...
function updateDniImage(isOld) {
    console.log('=== ACTUALIZANDO IMÁGENES DNI ===');
    console.log('isOld:', isOld);
    
    // Actualizar imagen del frente
    const dniImg = document.querySelector('.DNI_IMG');
    console.log('Elemento .DNI_IMG encontrado:', dniImg);
    if (dniImg) {
        const frontSrc = isOld ? 'imgs/arg_front_viejo.webp' : 'imgs/arg_front_new.webp';
        dniImg.src = frontSrc;
        console.log('Actualizando imagen del frente:', frontSrc);
        console.log('Nueva src del frente:', dniImg.src);
    } else {
        console.log('ERROR: No se encontró elemento .DNI_IMG');
    }
    
    // Actualizar imagen del dorso - probar múltiples selectores
    let dniBackImg = document.querySelector('.DNI_back img');
    if (!dniBackImg) {
        dniBackImg = document.querySelector('div.DNI_back img');
    }
    if (!dniBackImg) {
        dniBackImg = document.querySelector('[class*="DNI_back"] img');
    }
    
    console.log('Elemento .DNI_back img encontrado:', dniBackImg);
    if (dniBackImg) {
        const backSrc = isOld ? 'imgs/arg_back_viejo.webp' : 'imgs/arg_back_new.webp';
        dniBackImg.src = backSrc;
        console.log('Actualizando imagen del dorso:', backSrc);
        console.log('Nueva src del dorso:', dniBackImg.src);
    } else {
        console.log('ERROR: No se encontró elemento .DNI_back img');
        // Intentar con todos los elementos img
        const allImgs = document.querySelectorAll('img');
        console.log('Todas las imágenes encontradas:', allImgs);
        allImgs.forEach((img, index) => {
            console.log(`Imagen ${index}:`, img.src, img.alt);
        });
    }
...
function loadDniPreference() {
    ...
    // Si no hay preferencia guardada, usar DNI nuevo por defecto
    if (savedPreference === null) {
        return false; // false = DNI nuevo
    }
...
if (document.querySelector('input[name="oldDniBg"]')) {
    ...
    const radioToCheck = document.querySelector(`input[name="oldDniBg"][value="${isOld}"]`);
    if (radioToCheck) {
        radioToCheck.checked = true;
    }
    
    // Asegurar que las imágenes se actualicen al cargar la página
    ...
    document.querySelectorAll('input[name="oldDniBg"]').forEach(radio => {
        radio.addEventListener('change', function() {
            ...
            // Actualizar imagen inmediatamente en la misma página
            updateDniImage(isOld);
            // Disparar evento personalizado para otras partes de la página
            window.dispatchEvent(new CustomEvent('dniTypeChanged', { detail: { isOld } }));
        });
    });
}
...
if (document.querySelector('.DNI_IMG')) {
    ...
    window.addEventListener('dniTypeChanged', function(e) {
        updateDniImage(e.detail.isOld);
    });
}

// Función para forzar la actualización de imágenes cuando la página esté completamente cargada
document.addEventListener('DOMContentLoaded', function() {
    console.log('DOM cargado completamente');
    // Esperar un poco para asegurar que todos los elementos estén disponibles
    setTimeout(function() {
        const isOld = loadDniPreference();
        console.log('Forzando actualización de imágenes, isOld:', isOld);
        updateDniImage(isOld);
    }, 100);
});
